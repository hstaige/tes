...
class App(tk.Tk):
    def __init__(self):
        ...
        self.errOutLabel.grid(row=15, column=0, sticky='w')
        self.errOut = ConsoleText(master=self)
        self.errOut.grid(row=16,column=0, columnspan=5)
        self.errOut.start()
        self.load()
    ...
    def UpdatePlots(self):  #real-time plotting routine. refreshes data, adjust alphas, and replots graphs
        ...
        for s in range(len(States)):    #looping over each state passed in
            ...
            self.count_list.append(np.sum(self.rtpdata[s][1][(self.rtpdata[s][0]>800)&(self.rtpdata[s][0]<1300)]))
            print(f'{(self.count_list[self.updateIndex+1]-self.count_list[self.updateIndex])/int(self.RTPdelay.Val):.2f}')
